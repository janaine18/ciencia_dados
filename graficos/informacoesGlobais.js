...
async function visualizarInformacoesGlobais() {
  ...
  const trabalhadoresMundo = (dados.total_trabalhadores_mundo / 1e9).toFixed(2);
  ...
  paragrafo.innerHTML = `
    O mundo tem <span>${pessoasMundo}</span> bilhões de pessoas, dessas pessoas, aproximadamente 
    <span>${trabalhadoresMundo}</span> bilhões estão empregadas e passam em média 
    <span>${tempoDesTrabalho} horas</span> e <span>${minutos} minutos</span> por dia no caminho para o trabalho.
    Temos, portanto, mais da metade da população mundial que não exerce trabalhos com vínculos empregatícios legais.
    O que pode significar que há longevidade e, desse modo, muitas pessoas aposentadas.
